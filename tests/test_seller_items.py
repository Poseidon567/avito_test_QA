...
def test_get_seller_items(create_item):
    ...
    assert resp.status_code == 200, (f"Получение объявлений по sellerID закончилось"
    f" со статусом {resp.status_code}, ожидался статус 200")

    for item in resp.json():
        assert item["sellerId"] == seller_id, f"У объявления другой sellerID"
...
def test_get_seller_items_invalid_id(invalid_id):
    ...
    assert response.status_code == 400, (f"Получение объявлений по нечисловому sellerID закончилось"
...
def test_get_seller_items_empty_id():
    ...
    assert response.status_code == 400, (f"Получение объявлений по пустому sellerID закончилось"
    f" со статусом {response.status_code}, ожидался статус 400")
