...
def test_create_item_success(create_item):
    ...
    assert response.status_code == 200 , (f"Создание валидного объявления"
    ...
    assert data["sellerId"] == payload["sellerId"] , "В ответе поля sellerId  не совпадают"
    assert data["statistics"] == payload["statistics"] , "В ответе поля statistics  не совпадают"
...
def test_create_item_unique_id(create_item):
    ...
    assert resp1.status_code == 200 , (f"Создание первого объявления "
    f"закончилось со статусом {resp1.status_code}, ожидался статус 200")
    assert resp2.status_code == 200 , (f"Создание второго такого же "
...
@pytest.mark.parametrize("likes, view_count, contacts, null_field", [
    (0,1,1,"likes"),
    (1,0,1,"view_count"),
    (1,1,0,"contacts")
])
def test_create_item_zero_statistics(create_item, likes, view_count, contacts, null_field):
    response, _ = create_item(likes=likes, view_count=view_count, contacts=contacts)

    assert response.status_code == 200 , (f"Создание объявления с нулевым полем "
...
def test_create_item_zero_price(create_item):
    ...
    assert response.status_code == 200 , (f"Создание объявления с нулевой "
...
def test_create_item_empty_name(create_item):
    ...
    assert response.status_code == 400 , (f"Создание объявления с пустым "
...
def test_create_item_no_one_field(field):
    ...
    assert response.status_code == 400 , (f"Создание объявления без"
...
def test_create_item_negative_price(create_item):
    ...
    assert response.status_code == 400 , (f"Создание объявления с "
...
def test_create_item_float_price(create_item):
    ...
    assert response.status_code == 400 , (f"Создание объявления с нецелой "
...
@pytest.mark.parametrize("likes, view_count, contacts, neg_field", [
    (-1,1,1,"likes"),
    (1,-1,1,"view_count"),
    (1,1,-1,"contacts")
])
def test_create_item_negative_statistics(create_item, likes, view_count, contacts, neg_field):
    response, _ = create_item(likes=likes, view_count=view_count, contacts=contacts)
...
@pytest.mark.parametrize("likes, view_count, contacts, float_field", [
    (1.2,1,1,"likes"),
    (1,1.5,1,"view_count"),
    (1,1,1.8,"contacts")
])
def test_create_item_float_statistics(create_item, likes, view_count, contacts, float_field):
    response, _ = create_item(likes=likes, view_count=view_count, contacts=contacts)
